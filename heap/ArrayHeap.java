...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int parentLoc;
        if (index == 1 || current.myPriority > parent.myPriority) {
            return;
        } else {
            parentLoc = parentIndex(index);
    ...
    private void sink(int index) {
        ...
        Node current = contents[index];
        int leastChildLoc = min(leftIndex(index), rightIndex(index));
        Node leastChild = contents[leastChildLoc];
        if (current.myPriority <= leastChild.myPriority) {
            return;
        } else {
            swap(index, leastChildLoc);
        }
        sink(leastChildLoc);
    }

    private boolean isLeaf(int index) {
        return !inBounds(leftIndex(index)) || !inBounds(rightIndex(index));
    }
    ...
    public void insert(T item, double priority) {
        ...
        Node rightLeaf = new Node(item, priority);
        contents[size + 1] = rightLeaf;
        size += 1;
        swim(size);
    }
